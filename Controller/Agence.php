...
namespace AgencePhp\Controller;
...
class Agence
{
  ...
  public function __construct()
  {
    ...
    $this->oUtil = new \AgencePhp\Engine\Util;

    /** Récupère la classe Model dans toute la class controller **/
    $this->oUtil->getModel('Agence');
    $this->oModel = new \AgencePhp\Model\Agence;
  ...
  public function post()
  {
    if(empty($_GET['id']))
    {
      header('Location: Agence_index.html');
    }
  	...
  	if (isset($_POST['submit_comment']))
    {
        ...
        else
        {
          ...
          ?> <script>window.location.replace('Agence_post_<?= $_GET['id'] ?>.html');</script> <?php
  ...
  // On obtient tous les posts puis on affiche la page formulaire.php
  public function formulaire()
  {
    // $this->oUtil->oPosts = $this->oModel->getAll();

    $this->oUtil->getView('formulaire');
  }
	...
	{
			if ($this->isLogged())
					header('Location: ' . ROOT_URL . 'Agence_index.html');
	...
	public function registration()
	{
      ...
      $sPassword_again = htmlspecialchars(trim($_POST['password_again']));
			...
			else
			{
				...
				?> <script>window.location.replace('Agence_login.html');</script> <?php
	...
	public function signal ()
	{
		...
		header('Location: ' . ROOT_URL . 'Agence_post_' . $_GET['postid'] . '.html#comment_ink');
	}
